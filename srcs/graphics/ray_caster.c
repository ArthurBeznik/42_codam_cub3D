...
bool	draw_3d_walls(t_general_data *data, t_ray *raymond, int i, t_textures *textures)
{
	...
	*/
	...
	// fprintf(stderr, "[draw_3d_walls]\tdist_h: %f\n", raymond->dist_h); // ? testing
	if (ca < 0)
		ca += RESET_ANGLE;
	if (ca > 2 * M_PI)
		ca -= RESET_ANGLE;
	...
	textures->y = textures->y_off * textures->y_step + textures->hmt; // ! read next texture
	...
	while (n < line_h)
	{
		...
		// fprintf(stderr, "hmt | vmt : %d | %d\n", textures->hmt, textures->vmt); // ? testing
		...
		while (c < 8)
		{
			mlx_put_pixel(data->graphics->img_3d, x1 + c, y1, textures->rgba);
			c++;
		}
	...
	while (n < 320)
	{
		...
		// fprintf(stderr, "raFix: %f\n", raFix);
		textures->x = data->file_data->player->x / 2 + cos(deg) * 158 * 64 / dy / raFix; // ! 158 = 64 * 2 + 30
